...
$$
class \sim bernoulli(0.5)
y \sim Normal(\mu_c, \sigma_c | class=c)
$$
...
For simplicity we will assume that $\mu_1=-1$ and $\mu_1=1$ both with $\sigma=1$. 
We could also solve for unknown $\mu$'s, which would be the fully bayesian equivalent to the EM algorithm.
...
kl_divergence <- function(y, posterior_vals, mu_params, sigma_params){
  #calculate elbo
...
mu_steps <- as.data.frame(matrix(0, nrow = iters, ncol = 2)) %>% setNames(c("mu1", "mu2"))
kls <- c()
...
for(i in 1:iters){  
  ...
  for(j in 1:N){
    # get gradient
    gradient <- (kl_divergence(y[j], offset_vals[j,,drop=FALSE], mu_params, sigma_params) - 
      kl_divergence(y[j], posterior_vals[j,,drop=FALSE], mu_params, sigma_params)) / h
  ...
  # maximization
  # step mu1 params
  gradient1 <- (kl_divergence(y, posterior_vals, mu_params + h_ * h1, sigma_params) - 
    kl_divergence(y, posterior_vals, mu_params, sigma_params)) / h_
  mu_params <- mu_params - h1 * gradient1 * mu_step_size
  
  # step mu2 params
  gradient2 <- (kl_divergence(y, posterior_vals, mu_params + h_ * h2, sigma_params) - 
    kl_divergence(y, posterior_vals, mu_params, sigma_params)) / h_
  mu_params <- mu_params - h2 * gradient2 * mu_step_size

  # save intermediate values for later
  posterior_steps[i,] <- posterior_vals[,2]
  mu_steps[i,] <- mu_params
  kl <- kl_divergence(y, posterior_vals, mu_params, sigma_params)
  kls <- c(kls, kl)
}
```
...
learning_plot <- ggplot(steps_df, aes(y = value, x = y, color = class)) +
  geom_point() +
  geom_vline(data = mu_df, aes(xintercept = mu_value, color = mu)) +
...
for(i in 1:iters){
  ...
  mu_steps[i,] <- normal_network(c(1,2), decoder_params)
