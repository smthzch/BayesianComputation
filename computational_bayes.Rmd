...
This gives us an estimate of the mean of $\mu$ of about 1.59 with a standard deviation of about 1.08, so our posterior $p(\mu|x)$ is distributed:
...
If we use the same prior distribution and parameters as before $\mu_\mu=3,\sigma_\mu=2$,  that means we wish to draw samples that are normally distributed with mean 3.47 and standard deviation 1.08 without actually knowing those parameters.
...
estimates <- rbind(estimates, data.frame(method = "VI-Mismatch", mu_mu = exp(mu_beta_kl), mu_sigma = NA))
knitr::kable(estimates)
```
...
x1 <- seq(-5, 10, by = 0.1)
plot(y = dexp(x1, exp(mu_beta_kl)), x = x1, type = "l", lty = 1)
...
kl_divergence <- function(y, posterior_vals, mu_mu_params, mu_sigma_params, sigma_params){
  ...
  kl <- sapply(1:ndraws, function(i){
    ...
    p_mu <- sum(dnorm(mu_params, prior_mu, prior_sigma)) # mu prior
    q_z - p_y_z - p_z - p_mu
  })
  mean(kl)
}
